...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for d in all_dices:
        if d == 1:
            flag = True
    return 1 if flag else sum(all_dices)
    # END PROBLEM 1
...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    p, o = player_score % 10, opponent_score // 10 % 10
    score = 3 * abs(o - p)
...
def make_averaged(original_function, samples_count=1000):
    ...
    "*** YOUR CODE HERE ***"
    def fun_return(*args):
        result = [original_function(*args) for i in range(samples_count)]
        result = sum(result) / samples_count
        return result
    return fun_return
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    result = [average_score(i,dice) for i in range(1,11)]
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    p, score = score, sus_update(0,score,opponent_score)
