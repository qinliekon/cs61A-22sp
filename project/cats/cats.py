...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    re = [p for p in paragraphs if select(p)]
    if k >= len(re):
        return ''
    else:
        return re[k]
...
def accuracy(typed, source):
    ...
    lenth = max(len(typed_words),len(source_words))
    if lenth == 0:
        return 100.0
    ...
    return  s / lenth * 100 if len(typed_words) > len(source_words) else s / index * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    diff = [diff_function(typed_word,i,limit) for i in word_list]

    min_diff = word_list[diff.index(min(diff))]     
    '''取最小误差的索引值'''
    
    return typed_word if min(diff) > limit else min_diff
    # END PROBLEM 5
...
def minimum_mewtations(typed, source, limit):
    ...
    if limit < 0: # Base cases should go here, you may add more base cases as needed.
        return 0
    elif len(typed) * len(source) == 0: # Feel free to remove or add additional cases
...
def report_progress(typed, source, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    corr = 0
    for t, s in list(zip(typed,source)):
        if t == s:
            corr += 1
        else:
            break
    progress = corr / len(source)
...
def time_per_word_match(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    word , tpp = time_per_word(words,timestamps_per_player)
    return match(word,tpp)
    # END PROBLEM 10
