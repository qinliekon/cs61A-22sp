...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        sp = self.place
        dis = 0
        while not sp.is_hive:
            if len(sp.bees) > 0 and self.min_range <= dis <= self.max_range:
                break
            sp = sp.entrance
            dis += 1
        if sp.is_hive:
            return None
        return random_bee(sp.bees)
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health - amount <= 0:
            for s in lst:
                s.reduce_health(amount + self.damage)
        else:
            for s in lst:
                s.reduce_health(amount)
...
class ScubaThrower(ThrowerAnt):
    name = 'ScubaThrower' #一点错误，ok里面给的全称
    is_waterproof = True
    implemented = True
    food_cost = 6
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while curr_place is not None:
            ...
            and not curr_place.ant.ant_contained.buffed:
                curr_place.ant.ant_contained.damage = curr_place.ant.ant_contained.damage*2
